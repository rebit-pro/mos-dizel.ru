...
function validate(values: any, { setErrors }: any) {
  ...
  return authStore.login(trimmedUser, password.value).catch((error) => setErrors({ apiError: error }));
}
</script>
...
<template>
  ...
  <div class="mt-5 text-end">
    <v-divider />
    <v-btn variant="plain" to="/register1" class="mt-2 text-capitalize me-n2">Don't Have an account?</v-btn>
  </div>
</template>
<style lang="scss">
.custom-devider {
